...
export async function Header() {
  ...
  const hotlineRaw = siteConfig.contact.phones[0] ?? "";
  ...
  const navItems = [
    ...
    { href: "/price-list", label: t("priceList") },
  ];
    ...
    <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex min-w-0 items-center gap-2 font-semibold">
          <BrandMark />
          <span className="truncate text-base">{siteConfig.shortName}</span>
        </Link>
        ...
        <div className="hidden items-center gap-2 xl:gap-3 lg:flex">
          ...
          <Link href="/patient-portal" className={buttonVariants({ className: "gap-2" })}>
        ...
        <div className="flex shrink-0 items-center gap-2 lg:hidden">
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="w-72">
              <SheetTitle className="px-4 pt-4">{siteConfig.shortName}</SheetTitle>
